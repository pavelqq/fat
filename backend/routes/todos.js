...
router.put("/:id", auth, async (req, res) => {
  ...
  if (todo.uid !== req.user._id)
    return res.status(401).send("Задание не обновлено. Нет авторизации...");
...
router.patch("/:id", auth, async (req, res) => {
  ...
  if (todo.uid !== req.user._id)
    return res.status(401).send("Выполнение/невыполнение невозможно. Нет авторизации...");
...
router.delete("/:id", auth, async (req, res) => {
  ...
  if (todo.uid !== req.user._id)
    return res.status(401).send("Удаление задания невозможнно. Нет авторизации...");
